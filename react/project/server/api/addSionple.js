...
router.post('/',async (req,res)=>{
    ...
    const {uid,tags,title,context,isShare,imgData,topic} = req.body;
    // const uid = 'k3i297def';
    // const tags = '测试2';
    // const title = '测试测试';
    ...
    // 所选话题
    var topic_id = await query('SELECT topic_id FROM topic WHERE topic_name=?',[topic]);
    topic_id = JSON.parse(JSON.stringify(topic_id))[0].topic_id;
    ...
    if(imgData.length === 1){
        ...
        await query('insert into chapter (uid,title,tag_id,context,chdate,isShare,ch_headimg,topic_id) values(?,?,?,?,?,?,?,?)',[uid,title,tag_id,context,chdate,isShare,img_path,topic_id]);
